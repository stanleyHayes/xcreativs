...
func NewRouter(cfg *config.Config, log Logger, pool *pgxpool.Pool, identity domain.IdentityRepository, jwtGen *jwt.Generator) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(auth chi.Router) {
			...
			// Portal — non-engagement-scoped routes
			...
			// Per-engagement resources — every {id} route is guarded by engagement
			// ownership/membership (EngagementAccessMiddleware) to close the
			// cross-tenant IDOR; mutations additionally require the "write" scope.
			engAccess := auth.With(EngagementAccessMiddleware(pool))
			engWrite := auth.With(EngagementAccessMiddleware(pool), RequireScope("write"))

			engAccess.Get("/portal/engagements/{id}", handleGetEngagement(pool))
			engAccess.Get("/portal/engagements/{id}/dashboard", handleEngagementDashboard(pool))
			engAccess.Get("/portal/engagements/{id}/milestones", handleListMilestones(pool))
			engWrite.Post("/portal/engagements/{id}/milestones", handleCreateMilestone(pool))
			engWrite.Patch("/portal/engagements/{id}/milestones/{milestoneID}", handleUpdateMilestone(pool))
			engWrite.Delete("/portal/engagements/{id}/milestones/{milestoneID}", handleDeleteMilestone(pool))
			engAccess.Get("/portal/engagements/{id}/deliverables", handleListDeliverables(pool))
			engWrite.Post("/portal/engagements/{id}/deliverables", handleCreateDeliverable(pool))
			engWrite.Patch("/portal/engagements/{id}/deliverables/{deliverableID}", handleUpdateDeliverable(pool))
			engWrite.Delete("/portal/engagements/{id}/deliverables/{deliverableID}", handleDeleteDeliverable(pool))
			engAccess.Get("/portal/engagements/{id}/decisions", handleListDecisions(pool))
			engWrite.Post("/portal/engagements/{id}/decisions", handleCreateDecision(pool))
			engWrite.Patch("/portal/engagements/{id}/decisions/{decisionID}", handleUpdateDecision(pool))
			engWrite.Delete("/portal/engagements/{id}/decisions/{decisionID}", handleDeleteDecision(pool))
			engAccess.Get("/portal/engagements/{id}/risks", handleListRisks(pool))
			engWrite.Post("/portal/engagements/{id}/risks", handleCreateRisk(pool))
			engWrite.Patch("/portal/engagements/{id}/risks/{riskID}", handleUpdateRisk(pool))
			engWrite.Delete("/portal/engagements/{id}/risks/{riskID}", handleDeleteRisk(pool))
			engAccess.Get("/portal/engagements/{id}/stakeholders", handleListStakeholders(pool))
			engWrite.Post("/portal/engagements/{id}/stakeholders", handleCreateStakeholder(pool))
			engWrite.Patch("/portal/engagements/{id}/stakeholders/{sid}", handleUpdateStakeholder(pool))
			engWrite.Delete("/portal/engagements/{id}/stakeholders/{sid}", handleDeleteStakeholder(pool))
			engAccess.Get("/portal/engagements/{id}/activity", handleActivityFeed(pool))
			engAccess.Get("/portal/engagements/{id}/team", handleListTeamMembers(pool))
			engWrite.Post("/portal/engagements/{id}/team", handleCreateTeamMember(pool))
			engWrite.Delete("/portal/engagements/{id}/team/{memberID}", handleRemoveTeamMember(pool))
			engAccess.Get("/portal/engagements/{id}/tickets", handleListSupportTickets(pool))
			engWrite.Post("/portal/engagements/{id}/tickets", handleCreateSupportTicket(pool))
			engWrite.Patch("/portal/engagements/{id}/tickets/{ticketID}", handleUpdateSupportTicket(pool))
			engWrite.Delete("/portal/engagements/{id}/tickets/{ticketID}", handleDeleteSupportTicket(pool))
			engAccess.Get("/portal/engagements/{id}/budget", handleListBudgetLines(pool))
			engAccess.Get("/portal/engagements/{id}/invoices", handleListInvoices(pool))
			engWrite.Post("/portal/engagements/{id}/invoices", handleGenerateInvoice(pool))
			engAccess.Get("/portal/engagements/{id}/capabilities", handleListCapabilityDeliveries(pool))
			engWrite.Post("/portal/engagements/{id}/capabilities", handleCreateCapability(pool))
			engWrite.Patch("/portal/engagements/{id}/capabilities/{cid}", handleUpdateCapability(pool))
			engWrite.Delete("/portal/engagements/{id}/capabilities/{cid}", handleDeleteCapability(pool))
			engAccess.Get("/portal/engagements/{id}/demos", handleListDemoLinks(pool))
			engWrite.Post("/portal/engagements/{id}/demos", handleCreateDemoLink(pool))
			engAccess.Get("/portal/engagements/{id}/documents", handleListDocumentLibrary(pool))
			engAccess.Get("/portal/engagements/{id}/reports", handleListReports(pool))
			engAccess.Get("/portal/engagements/{id}/approvals", handleListApprovalWorkflows(pool))
			engWrite.Post("/portal/engagements/{id}/approvals", handleCreateApprovalWorkflow(pool))
			engAccess.Get("/portal/engagements/{id}/threads", handleListThreads(pool))
			engWrite.Post("/portal/engagements/{id}/threads", handleCreateThread(pool))

			// Sibling resources keyed by their own id — handlers resolve the owning
			// engagement and authorize internally (see invoice/demo/approval handlers).
		...
		api.Get("/feed/insights", handleInsightsFeed(pool))
