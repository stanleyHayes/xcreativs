  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold flex items-center gap-2"><Layers className="w-5 h-5 text-signal" /> Capability Lattice</h2>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1 px-3 py-1.5 text-sm bg-signal text-black rounded-md hover:bg-signal/90 transition"><Plus className="w-4 h-4" /> Add</button>
      </div>
      ...
      {showForm && (
        <div className="border border-white/10 rounded-lg p-4 mb-6 bg-white/[0.03]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input value={form.capability_name} onChange={(e) => setForm({ ...form, capability_name: e.target.value })} placeholder="Capability name" className="md:col-span-2 bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-signal" />
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-signal">
              {columns.map((c) => <option key={c.key} value={c.key}>{c.label}</option>)}
            </select>
            {form.status === "deferred" && (
              <input value={form.reason_deferred} onChange={(e) => setForm({ ...form, reason_deferred: e.target.value })} placeholder="Reason deferred" className="md:col-span-3 bg-white/5 border border-white/10 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-signal" />
            )}
          </div>
          <div className="flex justify-end mt-3">
            <button onClick={create} disabled={!form.capability_name.trim()} className="px-3 py-1.5 text-sm bg-signal text-black rounded-md hover:bg-signal/90 disabled:opacity-50">Create</button>
          </div>
        </div>
      )}
          ...
          <div key={col.key} className={`border ${col.color} rounded-lg p-3 bg-white/[0.02] min-h-[140px]`}>
            ...
            <div className="space-y-2">
              {items.filter((i) => i.Status === col.key).map((c) => (
                <div key={c.ID} className="group border border-white/10 rounded px-2.5 py-2 text-sm bg-white/[0.03]">
