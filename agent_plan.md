...
- **Current pass — Phase D hardening + UI/UX redesign (in progress, as of 13 Jun 2026):**
  ...
  - **Full product redesign + production hardening (31 Aug 2026 — DONE):** preserved the core `#0066CC` signal blue while replacing the previous mixed editorial/card language with the shared **Signal Infrastructure** system across both apps. Outfit + Geist Mono now drive display, body, and operational labels; rectilinear panels, indexed blue rails, asymmetric grids, responsive page banners, consistent controls, focus/press states, active loading shimmer, dark portal command surfaces, and a rebuilt marketing navigation/footer/home composition inherit across every public, auth, portal, engagement, partner, and admin route family. Added an accessible `AlertBridge` so 28 legacy form/admin failure paths render branded non-blocking notifications instead of native browser dialogs. Updated Next.js and the locked image/CSS dependency chain to patched releases; production dependency audit now reports **0 vulnerabilities** (full dev audit: one low Windows-only local Vite/esbuild advisory). Also corrected Vitest discovery so Playwright specs are no longer executed as unit tests.
    - **Card and surface redesign:** replaced the remaining one-size-fits-all rounded cards with purpose-built families: chamfered public folios (`card-x`), framed working panels (`panel-x`), open editorial/legal reading surfaces, dark portal instruments, signal-led metric modules, diagnostic empty states, and separated admin table rows. Work, Labs, Industries, Tools, and Partners now use asymmetric 12-column compositions, indexed identifiers, consistent signal-blue icon frames, and content-aware spans instead of repetitive equal tiles. The shared selectors preserve compatibility across the existing route inventory while giving each surface family distinct hierarchy and behavior.
    - **Verification:** marketing + portal lint **0 warnings/errors**; both TypeScript checks green; marketing Vitest **6/6** green; both Next.js 16.3.3 production builds green with all route families emitted; desktop/mobile browser checks on marketing long-form + portal landing show no horizontal overflow (`scrollWidth === clientWidth`). Live API-backed route content still requires the backend on `:8081`, as designed.
