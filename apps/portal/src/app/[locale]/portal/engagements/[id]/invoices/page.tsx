  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-display text-xl font-semibold tracking-tight">Invoices</h2>
        ...
        <div className="portal-panel-x mb-6 p-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="portal-field-x w-full" placeholder="0.00" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="date" value={form.due_date} onChange={(e) => setForm({ ...form, due_date: e.target.value })} className="portal-field-x w-full" />
            </div>
          </div>
          <div className="flex justify-end mt-3">
            <button onClick={generate} disabled={busy === "new" || !form.amount} className="portal-btn-x disabled:opacity-50">
      ...
      {invoices.length === 0 ? (
        ...
        <div className="space-y-3">
          {invoices.map((inv) => {
            ...
            return (
              <div key={inv.ID} className="portal-card-x flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-2">
                  {!link && inv.Status !== "paid" && (
                    <button onClick={() => makeLink(inv.ID)} disabled={busy === inv.ID} className="portal-btn-secondary-x disabled:opacity-50">
                      <Link2 className="w-3.5 h-3.5" /> Payment link
                    </button>
                  )}
                  {link && inv.Status !== "paid" && (
                    <a href={link} target="_blank" rel="noopener noreferrer" className="portal-btn-x">Pay</a>
                  )}
