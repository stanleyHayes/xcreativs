...
export default function PartnerOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="flex items-center gap-2 font-display text-3xl font-semibold tracking-tight"><ShoppingCart className="w-5 h-5 text-purple-400" /> Distribution Orders</h1>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Total Orders</p><p className="text-xl font-bold">{orders.length}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Fulfilled</p><p className="text-xl font-bold text-green-400">{fulfilled}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Order Value</p><p className="text-xl font-bold">{format(totalValue)}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Commission</p><p className="text-xl font-bold text-green-400">{format(totalCommission)}</p></div>
      </div>

      <div className="space-y-3">
        {orders.map((o) => (
          <div key={o.ID} className="portal-card-x p-4">
