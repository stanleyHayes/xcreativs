  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="flex items-center gap-2 font-display text-3xl font-semibold tracking-tight"><Users className="w-5 h-5 text-green-400" /> Referrals</h1>
        <button onClick={() => setShowForm(!showForm)} className="portal-btn-x">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Total Referrals</p><p className="text-xl font-bold">{referrals.length}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Converted</p><p className="text-xl font-bold text-green-400">{referrals.filter((r) => r.Status === "converted").length}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Potential Value</p><p className="text-xl font-bold">{format(totalPotential)}</p></div>
        <div className="portal-card-x p-4"><p className="text-xs text-white/50">Commission Earned</p><p className="text-xl font-bold text-green-400">{format(totalEarned)}</p></div>
      </div>

      {/* New referral form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="portal-panel-x space-y-4 p-5">
      ...
      <div className="space-y-3">
        {referrals.map((r) => {
          ...
          return (
            <div key={r.ID} className="portal-card-x p-4">
